...
| 퀄리티 | ROE | 높을수록 고득점 → 그대로 순위 | BPS ≤ 0 제거 |
| 퀄리티 | 부채비율 역수 | 낮을수록 좋음 → 역수 변환 후 순위 | 상위 1% 클리핑 |
...
class QualityFactor:
    """
    퀄리티 팩터 계산
    - ROE = EPS / BPS × 100  (pykrx에서 직접 ROE를 제공하지 않으므로 직접 계산)
    - 부채비율 역수 (외부 데이터 필요, 선택 사항)
    """
    ...
    def calculate(
        ...
        Args:
            ...
            debt_ratio:   부채비율 Series (선택, index=ticker)
                          pykrx는 부채비율을 제공하지 않으므로 별도 수집 필요
...
| 공통 종목 수 | 세 팩터 모두 유효한 종목만 편입 → 전체 유니버스보다 줄어듦 (정상) |
| 부채비율 | pykrx 미제공 → Phase 3 초기에는 ROE 100% 사용, 이후 재무제표 크롤링 추가 가능 |
