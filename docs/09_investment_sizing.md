...
# v2.0 최종 (프리셋 A: 핵심 추천)
factor_weights:
  value: 0.70       # 밸류 (PBR 50%, PCR 30%, 배당 20%)
  momentum: 0.30    # 모멘텀 (12M 60%, 6M 30%, 3M 10%)
  quality: 0.00     # 퀄리티 (복합 스코어 제외, F-Score>=4 필터 전용)
```

v2.0에서는 전략 프리셋 A(핵심)/B(보수적)/C(공격적) 3개로 통합되었습니다.
