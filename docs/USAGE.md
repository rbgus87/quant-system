...
| 설정 | 기본값 | 설명 |
|------|--------|------|
| 밸류 가중치 | 0.70 (70%) | PBR 50% + PCR 30% + 배당률 20% (DART 기반) |
| 모멘텀 가중치 | 0.30 (30%) | 멀티기간: 12M 60% + 6M 30% + 3M 10% (최근 1개월 제외) |
| 퀄리티 가중치 | 0.00 (0%) | OP/A + EY + F-Score (v2.0: 복합 스코어 제외, F-Score>=4 필터로만 사용) |
